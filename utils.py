...
def make_srt_endpoint(transcript_response):
    srt_endpoint = "https://api.assemblyai.com/v2/transcript/"
    srt_endpoint += transcript_response['id'] + '/srt'
    print(srt_endpoint)
    return srt_endpoint
...
def get_srt(srt_endpoint, header):
    # paragraphs_response = requests.get(srt_endpoint + "/paragraphs", headers=header)
    response = requests.get(srt_endpoint, headers=header)
#    paragraphs_response = paragraphs_response.json()
